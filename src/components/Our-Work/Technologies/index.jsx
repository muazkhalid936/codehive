  ...
  return (
    <div className="bg-[#010B17] text-white py-20">
      <div className="container flex flex-col md:flex-row justify-center gap-6">
        <div>
          <h1 className="text-3xl md:text-5xl font-gilroy">
            Technologies
            <span className="text-[#229dd9]"> We Used</span>
          </h1>
          <p className="w-full  text-sm md:text-base my-10 font-lato font-[400]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 pt-4 rounded-xl">
            {technologies.map((tech, index) => (
              <div 
                key={index} 
                className={`flex flex-col border border-[#343c45] py-5 items-center gap-4 ${index === 1 ? 'rounded-tl-lg' : ''}`}
              >
                <img
                  src={tech.image}
                  alt="image"
                ...
                />
                <h1 className="text-lg font-gilroy">{tech.title}</h1>
              </div>
            ))}
          </div>
        </div>
